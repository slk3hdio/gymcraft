...
public final class MobAttachmentGameTests {
    ...
    public static void standaloneServiceAndPersistence(GameTestHelper helper) {
        ...
        backpack.set(4, ItemResource.of(Items.DIAMOND), 3);

        assertTrue(helper, MobAttachmentService.has(mob, MobAttachments.AGENT_BACKPACK), "backpack was not attached");
        assertEquals(helper, 3L, backpack.getAmountAsLong(4), "standalone backpack content mismatch");
        ...
        assertEquals(helper, 3L, restoredBackpack.getAmountAsLong(4), "snapshot lost backpack content");
    ...
    public static void accessScopeHidesWithoutDeleting(GameTestHelper helper) {
        var mob = spawnAgent(helper, EntityType.ZOMBIE, new BlockPos(2, 1, 2));
        var backpack = MobAttachmentService.attach(mob, MobAttachments.AGENT_BACKPACK);
        backpack.set(0, ItemResource.of(Items.EMERALD), 5);

        MobAttachmentAccessScope disabled = MobAttachmentAccessScope.activate(mob, List.of());
        assertEquals(helper, 8, AgentInventoryLayout.resolve(mob).size(), "disabled scope exposed backpack");
        ...
        assertEquals(helper, 35, layout.size(), "enabled scope did not expose 27 backpack slots");
        assertEquals(helper, 5, layout.slot(8).getItem().getCount(), "enabled scope lost backpack content");
        assertTrue(helper, layout.slot(8).identity() instanceof LogicalSlotIdentity.MobBackpack,
            "slot 8 is not a backpack slot");
        enabled.deactivate(mob);

        assertEquals(helper, 8, AgentInventoryLayout.resolve(mob).size(), "deactivated scope still exposed backpack");
        assertEquals(helper, 5L, backpack.getAmountAsLong(0), "deactivation deleted backpack content");
        helper.succeed();
    }
    ...
    public static void environmentsChooseBackpackAccess(GameTestHelper helper) {
        var mob = spawnAgent(helper, EntityType.ZOMBIE, new BlockPos(2, 1, 2));
        Identifier simpleId = Identifier.fromNamespaceAndPath(GymCraft.MODID, "simple_mob_test");
        SimpleMobEnv simple = new SimpleMobEnv(simpleId, mob);
        assertEquals(helper, 35, AgentInventoryLayout.resolve(mob).size(), "simple_mob did not enable backpack");
        simple.close();
        assertTrue(helper, MobAttachmentService.has(mob, MobAttachments.AGENT_BACKPACK), "close deleted persistent backpack");

        Identifier parkourId = Identifier.fromNamespaceAndPath(GymCraft.MODID, "parkour_mob_test");
        ParkourMobEnv parkour = new ParkourMobEnv(parkourId, mob);
        assertEquals(helper, 8, AgentInventoryLayout.resolve(mob).size(), "parkour_mob exposed existing backpack");
        parkour.close();
        helper.succeed();
    }
    ...
    public static void backpackMenuBridge(GameTestHelper helper) {
        ...
        assertEquals(helper, 35, chestSlotId, "menu-owned slot did not start after backpack");
        observe(mob);
        assertEquals(helper, ActionStatus.COMPLETED,
    ...
    public static void backpackDropsWithoutEnvironment(GameTestHelper helper) {
        var mob = spawnAgent(helper, EntityType.ZOMBIE, new BlockPos(2, 1, 2));
        var backpack = MobAttachmentService.attach(mob, MobAttachments.AGENT_BACKPACK);
        backpack.set(0, ItemResource.of(Items.DIAMOND), 2);
        backpack.set(9, ItemResource.of(Items.EMERALD), 4);
        ...
        assertEquals(helper, 0L, backpack.getAmountAsLong(0), "first backpack slot was not cleared");
        assertEquals(helper, 0L, backpack.getAmountAsLong(9), "second backpack slot was not cleared");
    ...
    public static void resetRestoresBackpackAndScope(GameTestHelper helper) {
        ...
        helper.runAfterDelay(5, () -> {
            try {
                ...
                assertEquals(helper, 35, layout.size(), "reset did not reactivate backpack scope");
    ...
    private static final class BackpackTestEnv extends AbstractMcEnv {
        ...
        private BackpackTestEnv(net.minecraft.world.entity.Mob mob) {
            super(
                Identifier.fromNamespaceAndPath(GymCraft.MODID, "backpack_test"),
                mob,
                List.of(),
                List.of(),
