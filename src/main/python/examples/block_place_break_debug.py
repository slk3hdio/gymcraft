"""Debug block setting and breaking actions.
...
def choose_place_target(obs: Any, max_reach: float) -> Pos | None:
    ...
    for block in blocks:
        ...
        for pos in candidate_place_positions(support):
            if pos in occupied:
                continue
            ...
            if distance <= max_reach:
                print(
                    ...
                    f"support=({support[0]}, {support[1]}, {support[2]}) "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Debug set_block and break_block actions")
    ...
    parser.add_argument("--skip-break", action="store_true", help="Only run set_block")
    ...
    try:
        ...
        if not args.skip_place:
            for i in range(args.place_count):
                if place_pos is None:
                    target = choose_place_target(obs, args.max_reach)
                    if target is None:
        ...
        if break_pos is None:
            print("no break target; provide --break-pos X Y Z or allow set_block first")
            return
