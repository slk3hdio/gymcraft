...
`ProtoUseItem.slot_id` 必须显式设置（包括主手槽 `0`），只接受 Agent 装备槽和自带容器槽。
...
- 外部目标先检查加载状态、距离和遮挡，再执行与 LookAt 相同的瞬时转向。
  ...
  没有可命中轮廓的空气不是方块交互目标。
- 方块调用 `ItemStack.useOn`，实体仅接受有效的其他 `LivingEntity`，调用 `interactLivingEntity`。
  不优先触发开箱、交易、骑乘；物品返回 `PASS` 或 `FAIL` 时动作失败，不回退为自身使用。
- 自身使用保留当前方向。投掷物沿视线发射；“自身”并不强制投掷物命中自己。
- 食物、药水每次消费一件，返回 `RUNNING` 并等待原版消费完成。效果作用于受控 Mob，
  普通食物不增加饥饿值或额外回血。零时长消费立即完成；其他持续使用（弓、盾等）暂不支持。
- 使用后恢复临时借用的主手，结果优先回来源槽；放不下的产物按空主手、自带容器、世界掉落清算。
  瞬时玩家交互沿用现有桥接容量限制（最多 33 个自带容器槽）。
- 超时、新动作、组合动作失败、死亡、reset、环境关闭和停服会停止消费并清理临时状态，中断不提前触发消费效果。
  reset 后旧实体物品仍按环境既有规则删除。菜单下次刷新会读取最新 Agent 物品栏。
