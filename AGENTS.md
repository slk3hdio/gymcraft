...
- **三个自定义 NeoForge registry** (在 `RegistryKeys` 用 `RegistryBuilder` + `NewRegistryEvent` 定义):
  - `action_components` → `ActionComponents` (7 个控制器: `step_move`, `move_to`, `set_attack_target`, `attack_once`, `noop`, `break_block`, `set_block`; `break_block` 经 `MobHandSimulator` 用 FakePlayer 复用原版破坏逻辑, `set_block` 复用 /setblock 的 `BlockStateParser`/`BlockInput` 链路)
