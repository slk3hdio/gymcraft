...
- **组件会话状态**挂 Mob 的 NeoForge attachment（注册入口 `registry/ModAttachments`），如菜单会话 `LogicalMenuSession`；菜单按钮经 `gym/menu/MenuAdapters` 静态列表按菜单类查找适配器（首批：讲台/切石机/织布机）
